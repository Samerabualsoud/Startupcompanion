...
export interface TermDefinition {
  term: string;
  arabicTerm?: string;
  category: 'economics' | 'control' | 'protection' | 'governance' | 'exit' | 'fundraising' | 'captable' | 'legal' | 'duediligence' | 'metrics' | 'equity';
  plain: string;
  detail: string;
  redFlag: boolean;
...
export const TERM_SHEET_GLOSSARY: TermDefinition[] = [
  ...
  // Equity & ESOP (ZestEquity)
  {
    term: 'ESOP (Employee Stock Option Plan)',
    arabicTerm: 'خطة خيارات أسهم الموظفين',
    category: 'equity',
    plain: 'A program that gives employees the right to buy company shares at a fixed price in the future.',
    detail: 'ESOPs align employee incentives with company success. Employees receive options (not shares) that vest over time. Once vested, they can exercise the options to buy shares at the strike price set at grant date. Common pool size is 10-20% of fully diluted shares.',
    redFlag: false,
    example: 'Employee granted 10,000 options at $0.50 strike. After 4-year vest, company valued at $5/share. Employee exercises: pays $5,000, receives shares worth $50,000.',
    founderTip: 'Create your ESOP pool before your Series A. Investors will require it anyway, and doing it before the round avoids diluting yourself post-investment.',
  },
  {
    term: 'Vesting Schedule',
    arabicTerm: 'جدول الاستحقاق',
    category: 'equity',
    plain: 'The timeline over which an employee earns their equity — you do not get it all at once.',
    detail: 'Standard vesting is 4 years with a 1-year cliff: 25% vests after 12 months, then 1/48th per month for 36 more months. Vesting protects the company if a founder or employee leaves early. Unvested shares are forfeited.',
    redFlag: false,
    example: '4-year vest, 1-year cliff: 0 shares for months 1-11, 25% on month 12, then ~2.08%/month until month 48.',
    founderTip: 'All founders should have vesting schedules, even between themselves. This is the single most important protection against a co-founder leaving early.',
  },
  {
    term: 'Cliff',
    arabicTerm: 'فترة الانتظار',
    category: 'equity',
    plain: 'The minimum time you must stay at a company before any equity vests.',
    detail: 'The standard cliff is 1 year. If you leave before the cliff, you get zero equity. After the cliff, you receive the full cliff amount at once (typically 25%), then vesting continues monthly.',
    redFlag: false,
    example: '1-year cliff: leave at month 11 → 0 shares. Stay until month 12 → 25% of grant vests immediately.',
    founderTip: 'A 1-year cliff is standard and fair. Be wary of co-founders who push back on cliffs.',
  },
  {
    term: 'Fully Diluted Shares',
    arabicTerm: 'الأسهم المخففة بالكامل',
    category: 'equity',
    plain: 'The total number of shares if every option, warrant, and convertible security were exercised.',
    detail: 'Fully diluted = issued shares + unexercised options + unvested options + warrants + shares from SAFE/convertible note conversion. Ownership percentages are always calculated on a fully diluted basis.',
    redFlag: false,
    example: '10M issued shares + 2M option pool + 500K warrants + 1M SAFE conversion = 13.5M fully diluted shares.',
    founderTip: 'Always calculate your ownership on a fully diluted basis. Your undiluted percentage is meaningless to investors.',
  },
  {
    term: 'Option Pool',
    arabicTerm: 'مجموعة خيارات الأسهم',
    category: 'equity',
    plain: 'Shares reserved for future employee equity grants.',
    detail: 'Typically 10-20% of fully diluted shares. Investors often require a larger option pool before investing, which dilutes founders (not investors) before the round closes. This is called the option pool shuffle.',
    redFlag: true,
    redFlagReason: 'Investors may push for a large option pool (20%+) before the round, which dilutes founders pre-money. Negotiate the pool size carefully — only what you actually need for the next 12-18 months of hiring.',
    founderTip: 'Model out exactly how many options you need for planned hires over the next 18 months.',
  },
  {
    term: 'ISO (Incentive Stock Option)',
    arabicTerm: 'خيار الأسهم التحفيزي',
    category: 'equity',
    plain: 'A tax-advantaged employee stock option available only in the US — gains are taxed at capital gains rates, not income rates.',
    detail: 'ISOs can only be granted to employees (not contractors or advisors). They must be exercised within 90 days of leaving the company. The total value of ISOs that can vest in a single year is capped at $100,000.',
    redFlag: false,
    founderTip: 'ISOs are more tax-efficient for employees than NSOs. Consult a tax advisor before exercising ISOs — the AMT can create unexpected tax bills.',
  },
  {
    term: 'NSO (Non-Qualified Stock Option)',
    arabicTerm: 'خيار الأسهم غير المؤهل',
    category: 'equity',
    plain: 'A stock option that does not qualify for special tax treatment — gains are taxed as ordinary income.',
    detail: 'NSOs can be granted to employees, contractors, advisors, and board members. Unlike ISOs, the spread is taxed as ordinary income at exercise. NSOs are more flexible but less tax-efficient than ISOs.',
    redFlag: false,
    founderTip: 'Advisors and board members typically receive NSOs. Employees should prefer ISOs when possible for better tax treatment.',
  },
  {
    term: 'RSU (Restricted Stock Unit)',
    arabicTerm: 'وحدة الأسهم المقيدة',
    category: 'equity',
    plain: 'A promise to give an employee actual shares after a vesting period — no purchase required.',
    detail: 'Unlike options, RSUs do not require employees to pay a strike price. They simply receive shares after vesting. RSUs are taxed as ordinary income when they vest. Common at late-stage companies and public companies.',
    redFlag: false,
    founderTip: 'RSUs are better for late-stage companies. For early-stage startups, options are more tax-efficient because employees can exercise at a low strike price.',
  },
  {
    term: 'SAR (Stock Appreciation Right)',
    arabicTerm: 'حق تقدير الأسهم',
    category: 'equity',
    plain: 'The right to receive the increase in share value over a period, without buying actual shares.',
    detail: 'SARs pay out the difference between the strike price and the current share value, either in cash or shares. Common in jurisdictions where equity issuance is complex (e.g., Saudi Arabia).',
    redFlag: false,
    founderTip: 'SARs are popular in the MENA region as an alternative to traditional ESOPs, especially in Saudi Arabia where equity issuance to employees can be administratively complex.',
  },
  {
    term: 'OQAL Note (Sukuk-Style Convertible)',
    arabicTerm: 'صك أوقال القابل للتحويل',
    category: 'equity',
    plain: 'A Shariah-compliant convertible instrument that converts into equity at a future funding round, without interest.',
    detail: 'OQAL notes are the MENA equivalent of a SAFE or convertible note, structured to comply with Islamic finance principles. They use a profit-sharing (Mudarabah) or co-investment (Musharakah) structure instead of interest.',
    redFlag: false,
    founderTip: 'If raising from GCC investors or Islamic funds, OQAL notes are often preferred over SAFEs or convertible notes.',
  },
  {
    term: 'Dilution',
    arabicTerm: 'التخفيف',
    category: 'equity',
    plain: 'The reduction in your ownership percentage when new shares are issued.',
    detail: 'Every time you issue new shares — for a funding round, ESOP grants, or warrant exercises — existing shareholders own a smaller percentage of the company. Dilution is not inherently bad: a smaller percentage of a more valuable company can be worth more.',
    redFlag: false,
    example: 'You own 60% of 10M shares. You raise a round issuing 5M new shares. You now own 6M of 15M = 40%. You were diluted by 20 percentage points.',
    founderTip: 'Model dilution across all future rounds before signing any term sheet.',
  },
  {
    term: 'Anti-Dilution Protection',
    arabicTerm: 'الحماية من التخفيف',
    category: 'equity',
    plain: 'A clause that protects investors from dilution if you raise a future round at a lower valuation (a down round).',
    detail: 'Two main types: (1) Full Ratchet — very founder-unfriendly. (2) Broad-Based Weighted Average — price is adjusted based on the size of the down round, more common and fair.',
    redFlag: true,
    redFlagReason: 'Full ratchet anti-dilution is extremely founder-unfriendly. It can wipe out founders in a down round. Always push for broad-based weighted average anti-dilution instead.',
    founderTip: 'Broad-based weighted average anti-dilution is the industry standard. Full ratchet is a red flag — push back hard.',
  },
  {
    term: 'Accelerated Vesting',
    arabicTerm: 'الاستحقاق المتسارع',
    category: 'equity',
    plain: 'Unvested equity that vests immediately upon a trigger event, such as an acquisition.',
    detail: 'Two types: (1) Single Trigger — vesting accelerates upon acquisition alone. (2) Double Trigger — vesting accelerates only if there is an acquisition AND the employee is terminated. Double trigger is more common and preferred by acquirers.',
    redFlag: false,
    founderTip: 'Negotiate double-trigger acceleration for yourself and key employees. This protects you if you are fired after an acquisition.',
  },
];
