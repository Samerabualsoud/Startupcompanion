  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {arrFromSalesBridge && !isComplete && (
        <div className="flex items-center gap-2 px-4 py-2 text-[11px] font-medium" style={{ background: 'var(--primary)', color: 'var(--muted-foreground)', borderBottom: '1px solid #5B4EFF' }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        <AnimatePresence initial={false}>
          {messages.map(msg => (
            ...
            >
              {msg.role === 'bot' && (
                <div className="w-7 h-7 rounded-full shrink-0 mr-2 flex items-center justify-center text-xs" style={{ background: 'var(--primary)', color: 'var(--foreground)' }}>
          ...
          {isTyping && (
            <motion.div key="typing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex justify-start">
              <div className="w-7 h-7 rounded-full shrink-0 mr-2 flex items-center justify-center" style={{ background: 'var(--primary)' }}>
                <Sparkles className="w-3.5 h-3.5" style={{ color: 'var(--primary)' }} />
              </div>
      ...
      {isInferring && (
        <div className="border-t border-border p-4 bg-card flex items-center gap-2 text-xs text-muted-foreground">
          <div className="w-4 h-4 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: 'var(--border)', borderTopColor: 'transparent' }} />
      ...
      {aiInferredFields.length > 0 && !isComplete && (
        <div className="px-4 py-2 flex items-start gap-2 text-[10px] text-muted-foreground" style={{ background: 'rgba(91, 78, 255, 0.06)' }}>
          <Info className="w-3 h-3 shrink-0 mt-0.5" style={{ color: 'var(--primary)' }} />
