...
import {
  runValuation, defaultInputs, formatCurrency, formatPercent,
  SECTOR_MULTIPLES, STAGE_MEDIAN_VALUATIONS,
  type StartupInputs, type ValuationSummary,
} from '@/lib/valuation';
...
import {
  Info, ChevronDown, ChevronRight, Download, RefreshCw,
  TrendingUp, AlertTriangle, CheckCircle, Activity, DollarSign,
...
const METHOD_COLORS = [
  '#C4614A', '#0F1B2D', '#8B4A38', '#2D4A6B', '#A0522D', '#1B3A5C', '#D4845A',
];
...
function InputField({
  label, value, onChange, min = 0, max, step = 1, prefix = '', suffix = '', tooltip, type = 'number'
}: {
  ...
  return (
    <div className="mb-3">
      ...
      <select
        ...
        className="vc-input w-full text-sm py-2 px-3"
      >
...
function RiskSlider({ label, value, onChange, tooltip }: {
...
function ValuationChart({ summary }: { summary: ValuationSummary }) {
  const data = summary.results.map((r, i) => ({
    name: r.method.replace('Method', '').replace('Comparable Transactions', 'Comps').replace('Discounted Cash Flow', 'DCF').replace('First Chicago', 'First Chicago').trim(),
    value: Math.round(r.value / 1e6 * 100) / 100,
    low: Math.round(r.low / 1e6 * 100) / 100,
    high: Math.round(r.high / 1e6 * 100) / 100,
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 60 }}>
          ...
          <XAxis
            dataKey="name"
            tick={{ fontSize: 10, fontFamily: 'DM Sans', fill: 'oklch(0.45 0.02 240)' }}
            angle={-35}
            textAnchor="end"
            interval={0}
          />
          <YAxis
            tick={{ fontSize: 10, fontFamily: 'JetBrains Mono', fill: 'oklch(0.45 0.02 240)' }}
            tickFormatter={v => `$${v}M`}
          />
          <RechartTooltip
            formatter={(v: any, name: string) => [`$${v}M`, name]}
            contentStyle={{ fontSize: 11, fontFamily: 'DM Sans', borderRadius: 4, border: '1px solid oklch(0.88 0.01 80)' }}
          />
          ...
          <Bar dataKey="value" radius={[3, 3, 0, 0]}>
            {data.map((entry, i) => (
              <Cell key={i} fill={entry.color} opacity={0.85 + entry.applicability / 1000} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
...
function ScorecardRadar({ inputs }: { inputs: StartupInputs }) {
  ...
  return (
    <div>
      <div className="text-xs font-semibold text-foreground mb-2" style={{ fontFamily: 'Playfair Display, serif' }}>
        Scorecard Radar
      </div>
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-secondary text-muted-foreground font-mono">
            {result.applicability}% fit
          </span>
        </div>
        <div className="flex items-center gap-3">
          <span className="metric-value text-base font-bold" style={{ color }}>
            ...
            <div className="px-3 pb-3 border-t border-border">
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-mono text-foreground">
                  {formatCurrency(result.low, true)} — {formatCurrency(result.high, true)}
                </span>
              </div>
              ...
              <div className="mb-3">
                ...
                <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${result.confidence}%`, backgroundColor: color }}
                  />
                </div>
              </div>
                ...
                {Object.entries(result.breakdown).map(([k, v]: [string, any]) => (
                  <div key={k} className="text-[10px]">
                    ...
                    <span className="font-mono font-semibold text-foreground">
                      {typeof v === 'number' ? formatCurrency(v, true) : v}
                    </span>
                  </div>
                ))}
              </div>
...
export default function Home() {
  ...
  const [openSections, setOpenSections] = useState<Set<string>>(new Set(['company', 'financials']));
  ...
  const update = useCallback(<K extends keyof StartupInputs>(key: K, value: StartupInputs[K]) => {
    setInputs(prev => {
      ...
      // Auto-update sector multiples when sector changes
      if (key === 'sector') {
      ...
      // Auto-update median valuation when stage changes
      if (key === 'stage') {
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'oklch(0.978 0.008 80)' }}>
      ...
      >
        <div
          ...
          style={{
            backgroundImage: `url(https://d2xsxph8kpxj0f.cloudfront.net/105753932/M2SxUioygz4pYLdarmBijQ/hero-bg-6NSfALEUoBoxyRfotDh8fm.webp)`,
        ...
        <div className="relative container py-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              <div className="section-stamp text-xs mb-2" style={{ color: 'oklch(0.55 0.13 30)' }}>
                Multi-Method Analysis Tool
              </div>
              <h1
              ...
              <p className="text-sm mt-2" style={{ color: 'oklch(0.72 0.02 240)' }}>
                7 industry-standard methods · Blended confidence scoring · Live sensitivity analysis
              </p>
            </div>
            ...
            >
              ...
              <div
                ...
                style={{ color: 'oklch(0.978 0.008 80)', fontFamily: 'JetBrains Mono, monospace' }}
              >
                {formatCurrency(summary.blended, true)}
              </div>
              <div className="flex items-center gap-3 mt-2">
                <div className="text-[10px] font-mono" style={{ color: 'oklch(0.72 0.02 240)' }}>
              ...
              <div className="mt-2">
                <div className="flex justify-between text-[10px] mb-1" style={{ color: 'oklch(0.72 0.02 240)' }}>
                  <span>Confidence</span>
                  <span className="font-mono">{summary.confidenceScore}%</span>
                </div>
                <div className="h-1 rounded-full" style={{ background: 'oklch(0.28 0.04 240)' }}>
                  <div
                    className="h-full rounded-full transition-all duration-500"
                    style={{ width: `${summary.confidenceScore}%`, background: 'oklch(0.55 0.13 30)' }}
                  />
                </div>
              </div>
            </motion.div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside
          className="hidden lg:flex flex-col w-52 shrink-0 border-r border-border/30 py-4"
          style={{ background: 'oklch(0.18 0.05 240)' }}
        >
          <div className="px-4 mb-4">
            <div className="text-[10px] font-mono uppercase tracking-widest" style={{ color: 'oklch(0.45 0.04 240)' }}>
              Input Sections
            </div>
          </div>
          {SECTIONS.map(s => (
            <button
              ...
              className={`flex items-center gap-2.5 px-4 py-2.5 text-left text-xs transition-all ${
                activeSection === s.id
                  ? 'border-l-2 border-accent text-white bg-white/5'
        ...
        <div className="flex-1 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 240px)' }}>
          <div className="container py-6 max-w-2xl">
            ...
            <div id="section-company" className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <SelectField
                  label="Stage"
                  value={inputs.stage}
                  onChange={v => update('stage', v as any)}
                  options={[
                  ...
                  tooltip="Funding stage affects method applicability weights and median valuations."
                />
                <SelectField
                  label="Sector"
                  value={inputs.sector}
                  onChange={v => update('sector', v as any)}
                  options={[
                  ...
                  tooltip="Sector auto-populates comparable revenue multiples."
                />
              </div>
            </div>
            ...
            <div id="section-financials" className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <InputField label="Current ARR" value={inputs.currentARR} onChange={v => update('currentARR', v)} prefix="$" step={10000} tooltip="Annual Recurring Revenue — the annualized value of your recurring subscription contracts." />
                ...
                <InputField label="Gross Margin" value={inputs.grossMargin} onChange={v => update('grossMargin', v)} suffix="%" max={100} tooltip="Revenue minus COGS, expressed as a percentage of revenue." />
                <InputField label="Monthly Burn Rate" value={inputs.burnRate} onChange={v => update('burnRate', v)} prefix="$" step={5000} tooltip="Net monthly cash outflow (expenses minus revenue)." />
                <InputField label="Cash on Hand" value={inputs.cashOnHand} onChange={v => update('cashOnHand', v)} prefix="$" step={100000} tooltip="Current cash and equivalents on the balance sheet." />
                <InputField label="TAM ($)" value={inputs.totalAddressableMarket} onChange={v => update('totalAddressableMarket', v)} prefix="$" step={100000000} tooltip="Total Addressable Market — the total revenue opportunity available." />
                <div className="col-span-2">
                  <InputField label="Projected Revenue (5Y)" value={inputs.projectedRevenue5Y} onChange={v => update('projectedRevenue5Y', v)} prefix="$" step={500000} tooltip="Your best estimate of annual revenue in 5 years. Used in VC Method and First Chicago." />
                </div>
              </div>
            </div>
            ...
            <div id="section-dcf" className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <InputField label="Discount Rate (WACC)" value={inputs.discountRate} onChange={v => update('discountRate', v)} suffix="%" tooltip="Weighted Average Cost of Capital. Early-stage startups typically use 30–50%." />
                <InputField label="Terminal Growth Rate" value={inputs.terminalGrowthRate} onChange={v => update('terminalGrowthRate', v)} suffix="%" tooltip="Long-term sustainable growth rate. Typically 2–4% (GDP-level)." />
              </div>
              <div className="mt-2">
                <label className="block text-xs font-medium text-muted-foreground mb-2">
                  5-Year Projected Free Cash Flows
                  <InfoTip text="Projected free cash flows for each of the next 5 years. Negative values are acceptable for early-stage startups." />
                </label>
                  ...
                  {inputs.projectedCashFlows.map((cf, i) => (
                    <div key={i}>
                      ...
                      <input
                        ...
                        value={cf}
                        step={50000}
                        onChange={e => updateCF(i, parseFloat(e.target.value) || 0)}
                        className="vc-input w-full text-xs py-1.5 px-2 font-mono text-center"
                      />
                    </div>
                  ))}
                </div>
            ...
            <div id="section-scorecard" className="mb-6">
              ...
              <div className="mb-3 p-3 rounded-md bg-secondary/50 text-xs text-muted-foreground leading-relaxed">
                Rate each dimension 0–100 vs. a comparable startup. 100 = exceptional, 50 = average, 0 = poor.
              </div>
              ...
              <InputField label="Median Pre-Money Valuation ($)" value={inputs.medianPreMoneyValuation} onChange={v => update('medianPreMoneyValuation', v)} prefix="$" step={100000} tooltip="Regional/sector median pre-money valuation for comparable startups at this stage." />
            ...
            <div id="section-berkus" className="mb-6">
              ...
              <div className="mb-3 p-3 rounded-md bg-secondary/50 text-xs text-muted-foreground leading-relaxed">
                Assign up to $500K per milestone achieved. Best for pre-revenue startups. Max valuation: $2.5M.
              </div>
              <div className="grid grid-cols-1 gap-2">
                <InputField label="Sound Idea (Reduces Basic Risk)" value={inputs.berkusSoundIdea} onChange={v => update('berkusSoundIdea', clamp(v, 0, 500000))} prefix="$" max={500000} step={25000} tooltip="Is the core idea validated? Does it solve a real problem?" />
                ...
                <InputField label="Strategic Relationships (Reduces Market Risk)" value={inputs.berkusStrategicRelationships} onChange={v => update('berkusStrategicRelationships', clamp(v, 0, 500000))} prefix="$" max={500000} step={25000} tooltip="Are there key partnerships, LOIs, or distribution agreements?" />
            ...
            <div id="section-vcmethod" className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <InputField label="Target Return Multiple" value={inputs.targetReturn} onChange={v => update('targetReturn', v)} suffix="x" tooltip="The return multiple the VC expects on their investment (e.g., 10x)." />
                ...
                <InputField label="Years to Exit" value={inputs.yearsToExit} onChange={v => update('yearsToExit', v)} tooltip="Expected number of years until liquidity event." />
            ...
            <div id="section-comps" className="mb-6">
              ...
              <div className="mb-2 text-xs text-muted-foreground">
                Sector auto-populates these values. Adjust based on your specific comparables.
              </div>
            ...
            <div id="section-risk" className="mb-6">
              ...
              <div className="mb-3 p-3 rounded-md bg-secondary/50 text-xs text-muted-foreground leading-relaxed">
                Rate each factor: −2 (very negative) to +2 (very positive). Each unit = ±$250K adjustment to median.
              </div>
              <div className="grid grid-cols-1 gap-1">
                <RiskSlider label="Management Team" value={inputs.riskManagement} onChange={v => update('riskManagement', v)} tooltip="Quality and completeness of the management team." />
                <RiskSlider label="Stage of Business" value={inputs.riskStage} onChange={v => update('riskStage', v)} tooltip="How far along is the company in its development?" />
                <RiskSlider label="Legislation / Political Risk" value={inputs.riskLegislation} onChange={v => update('riskLegislation', v)} tooltip="Regulatory environment and political risk exposure." />
                <RiskSlider label="Manufacturing Risk" value={inputs.riskManufacturing} onChange={v => update('riskManufacturing', v)} tooltip="Ability to produce and deliver the product at scale." />
                <RiskSlider label="Sales & Marketing Risk" value={inputs.riskSalesMarketing} onChange={v => update('riskSalesMarketing', v)} tooltip="Effectiveness of go-to-market strategy." />
                <RiskSlider label="Funding / Capital Risk" value={inputs.riskFunding} onChange={v => update('riskFunding', v)} tooltip="Risk of running out of capital before profitability." />
                <RiskSlider label="Competition Risk" value={inputs.riskCompetition} onChange={v => update('riskCompetition', v)} tooltip="Intensity and quality of competitive landscape." />
                <RiskSlider label="Technology Risk" value={inputs.riskTechnology} onChange={v => update('riskTechnology', v)} tooltip="Technical feasibility and IP protection." />
                <RiskSlider label="Litigation Risk" value={inputs.riskLitigation} onChange={v => update('riskLitigation', v)} tooltip="Exposure to lawsuits, IP disputes, or regulatory actions." />
                <RiskSlider label="International Risk" value={inputs.riskInternational} onChange={v => update('riskInternational', v)} tooltip="Exposure to international market risks." />
                <RiskSlider label="Reputation Risk" value={inputs.riskReputation} onChange={v => update('riskReputation', v)} tooltip="Brand strength and reputational resilience." />
                <RiskSlider label="Lucrative Exit Potential" value={inputs.riskPotentialLucrative} onChange={v => update('riskPotentialLucrative', v)} tooltip="Likelihood and size of a profitable exit." />
              </div>
            </div>
            ...
            <div id="section-scenarios" className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                <InputField label="Exit Multiple" value={inputs.scenarioExitMultiple} onChange={v => update('scenarioExitMultiple', v)} suffix="x" tooltip="Revenue multiple applied at exit for all scenarios." />
                <InputField label="Discount Rate" value={inputs.scenarioDiscountRate} onChange={v => update('scenarioDiscountRate', v)} suffix="%" tooltip="Discount rate applied to all scenario terminal values." />
                <InputField label="Years to Exit" value={inputs.scenarioYearsToExit} onChange={v => update('scenarioYearsToExit', v)} tooltip="Investment horizon for all scenarios." />
              </div>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-2 text-xs text-muted-foreground hover:text-accent transition-colors mb-8"
            >
        ...
        {/* ── Results Panel ── */}
        <div
          className="hidden xl:flex flex-col w-[480px] shrink-0 border-l border-border overflow-y-auto"
          style={{ maxHeight: 'calc(100vh - 240px)', background: 'oklch(0.993 0.003 80)' }}
        >
          <div className="p-5">
            {/* Chart */}
            ...
            <div className="mt-4">
              ...
              <div className="space-y-2">
                {summary.results.map((r, i) => (
                  <MethodCard key={r.methodCode} result={r} index={i} />
                ))}
              </div>
            </div>

            {/* Interpretation */}
            <div className="mt-5 p-4 rounded-md border border-border bg-card">
              <h3 className="text-sm font-semibold mb-2" style={{ fontFamily: 'Playfair Display, serif' }}>
                Analyst Notes
              </h3>
              <div className="space-y-2 text-xs text-muted-foreground leading-relaxed">
                <p>
                  <strong className="text-foreground">Blended Valuation:</strong>{' '}
                  {formatCurrency(summary.blended, true)} — weighted by method applicability to {summary.stage} stage.
                </p>
                <p>
                  <strong className="text-foreground">Runway:</strong>{' '}
                  {summary.runway === 999 ? 'Profitable / no burn' : `${summary.runway} months`} at current burn rate.
                </p>
                <p>
                  <strong className="text-foreground">Burn Multiple:</strong>{' '}
                  {summary.burnMultiple}x — {summary.burnMultiple < 1 ? 'excellent capital efficiency' : summary.burnMultiple < 2 ? 'good efficiency' : summary.burnMultiple < 4 ? 'moderate — room for improvement' : 'high burn relative to growth'}.
                </p>
                <p>
                  <strong className="text-foreground">Implied ARR Multiple:</strong>{' '}
                  {summary.impliedARRMultiple}x — {summary.impliedARRMultiple > 20 ? 'premium growth-stage multiple' : summary.impliedARRMultiple > 10 ? 'strong SaaS multiple' : 'conservative / early-stage discount applied'}.
                </p>
                <p>
                  <strong className="text-foreground">Risk Level:</strong>{' '}
                  <span style={{ color: riskColor }}>{riskLevel}</span> — based on burn multiple and stage.
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 p-3 rounded-md bg-secondary/50 text-[10px] text-muted-foreground leading-relaxed">
              <strong>Disclaimer:</strong> This tool provides estimates for educational and planning purposes only. Actual valuations depend on negotiation, market conditions, investor thesis, and due diligence findings. Consult a qualified financial advisor before making investment decisions.
            </div>
          </div>
        </div>
      </div>

      {/* ── Mobile Results (below inputs on small screens) ── */}
